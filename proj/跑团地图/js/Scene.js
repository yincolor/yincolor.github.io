...
class Scene {
    ...
    async loadMapImage(file) {
        const _map_image = await utils.readImageFile(file);
        if (_map_image) {
    ...
    setCanvasEventHandle() {
        ...
        canvas.addEventListener('wheel', (ev) => {
            if (ev.deltaY < 0) { // 放大
                ...
                this.img_scale -= (this.img_scale >= 0.1 ? 0.1 : 0);
            }
...
class Character {
    ...
    draw() {
        ...
        const font_width = ctx.measureText(this.name).width + 2; 
        ...
        ctx.strokeText(this.name, name_x, name_y);
        ctx.stroke();
    }
